...
pygame.display.set_caption('Bot 8 Algo')
...
def refactor_probs(initial_node, track_path, track_beep, old_prob_dist):
    ...
    remove_beeps = math.ceil(beep_count * 0.516)
...
def generate_alien_pairs(grid,blocked_nodes,open_pairs):
    ...
    for i in range(len(open_pairs)):
        ...
        part3.append(0)
        dist_between_aliens=grid_distances[open_pairs[i][0]][open_pairs[i][1]]
        if dist_between_aliens in range(39 , 51):
            part4.append(.34)
        elif dist_between_aliens in range( 6, 71):
            part4.append(.136)
...
def update_alien_pairs(x,y,blocked_nodes,alien_pairs,prev,crew_found_inside_detection_grid,neigbhours_dic):
    ...
    if oper=="inside":
        ...
        for a in range(len(newpart2)):
            newpart2[a]=newpart2[a]*part3[a]

...
# Function to update probabilty distribution of crew member when a beep is heard
...
# Function to update the probability distribution of the crew member when no beep is heard
...
# Function to update the alien probability distribution once the alien has moved
...
# Function to update the alien probability distribution once the bot has moved
def update_crew_alien_grid_BotMoved(grid, crew_alien_grid, x, y, prev, crew_found_inside_detection_grid, blocked_nodes,movement):
    copy_crew_alien_grid = copy.deepcopy(crew_alien_grid)
    crew_alien_grid = [[0 for i in range(size)] for j in range(size)]
...
# Function to generate all the possible moves an alien can make from a given cell
...
# Function to check whether the alien is inside the detection box
...
# Function to calculate the probability of an alien entering the bot cell
...
# Blocking the current cell probability becuase it does not have a crew member
prob_mat[initial_node[0]][initial_node[1]] = -1

prev_node = initial_node
f=open('bot2Analysis.csv','a')
writer=csv.writer(f)
ans = 0
while simulate:
    flag=0
    while initial_node != desired_node:
        flag=0
        time_step+=1
        # Function to display current grid configuration
        visualizeGrid(initial_node, aliens)
        time.sleep(0.2)
        # Checking whether the alien is inside the detection square
        ...
        # Storing what the detection output was at the previous time step
        prev = crew_found_inside_detection_grid
        prev_node=initial_node
        
        
        # calculating distance to crew member
        d = len(astar(grid, initial_node, desired_node, aliens))
        ans = beep_prob(alpha, d)[0]
        # function to pre calculated distances in grid (used during probability updation)
        if initial_node not in grid_distances:
            calc_distances(initial_node)
        # Updating crew probability distribution according to beep
        if ans == 1:
        ...
        if initial_node in aliens:
            ...
            writer.writerow([alpha,crew_alien_grid_size,alien_threshold,time_step,idle_time,"alien Found"])
            f.close()
            sys.exit(1)
        # Function to see if the alien has entered the detection box
        crew_found_inside_detection_grid = find_crew_found_inside_detection_grid(grid, aliens, initial_node[0], initial_node[1])
        # Function to change the alien probability distribution after the aliens have moved
        ...
        # checking whether our current bot position is acceptable
        ...
        # checking for which cell in the grid has maximum probability of having a crew member
        max_prob = -1
        max_nodes = []
        for i in range(size):
            for j in range(size):
        ...
        # choosing one node out of all the nodes that have maximum probability of crew member
        move_to = random.choice(max_nodes)
        path = astar(grid, initial_node, move_to, aliens)
        # moving the bot towards the cell with maximum probability of having crew member
        initial_node = path[1]
        prob_mat[initial_node[0]][initial_node[1]] = -1
        if initial_node in aliens:
            # bot enters an alien occupied cell
            writer.writerow([alpha,crew_alien_grid_size,alien_threshold,time_step,idle_time,"alien Found"])
            f.close()
            sys.exit(1)
        if initial_node == desired_node:
            # crew member is found
            writer.writerow([alpha,crew_alien_grid_size,alien_threshold,time_step,idle_time,"alien Found"])
            f.close()
            sys.exit(1)
        # The probabitliy of the crew member not being present in the current cell
        factor = (1 - prob_mat[initial_node[0]][initial_node[1]])
        # blocking current cell probability value because it does not have a crew member
        prob_mat[initial_node[0]][initial_node[1]] = -1
        # distributing the probability of the current cell to the whole grid

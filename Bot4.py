...
# Function to calculate the distance of all other open cells from our current cell
...
# Function to update the alien probaility distribution once the alien has moved
...
# Checking for all the possible moves that an alien can make
...
# Function to check if the alin is inside the detection box
...
while simulate:
    ...
    while len(desired_nodes) > 0:
        ...
        # Checking if an alien entered the detection square
        crew_found_inside_detection_grid = find_crew_found_inside_detection_grid(grid, aliens, initial_node[0],initial_node[1])
        # Updating the alien probability distribution once the bot has moved
        ...
        # Checking if an alien entered our detection box
        crew_found_inside_detection_grid = find_crew_found_inside_detection_grid(grid, aliens, initial_node[0],initial_node[1])
        # Updating the alien probaility distribution once the alien moves
        ...
        # Checking if the is acceptable for the bot to be present in the current position
        ...
        if len(desired_nodes) == 2:    
            ...
            for i in open_pairs:
                if i[0] != initial_node and i[1] != initial_node:
                    if pair_probs[i[0][0]][i[0][1]][i[1][0]][i[1][1]] > max_prob:
                        ...
                        max_pairs.append(i)
            # Choosing the pair that has the maximum probability of crew member being present in it
            ...
            # Checking which cell has a higher probability of crew member being present in it
            if p1 > p2:
            ...
            for i in range(initial_node[0] + 1, size):
                for j in range(initial_node[1] + 1, size):
                    ...
                    if pair_probs[i][j][initial_node[0]][initial_node[1]] > 0:
                        cell_prob += pair_probs[i][j][initial_node[0]][initial_node[1]]
            # Probability of the crew member not begin present in the current cell
            factor = 1 - cell_prob
            if initial_node in desired_nodes:
                desired_nodes.remove(initial_node)
                if len(desired_nodes) == 0:
        ...
        # Block to be executed when there is one crew member in the grid
            ...
            # Choosing the cell to move towards in the grid
            move_to = random.choice(max_nodes)
            path = astar(grid, initial_node, move_to, aliens)
            # Moving the bot towards the maximum probability cell in the grid
            initial_node = path[1]
            ...
            if initial_node == desired_node:
                ...
                f.close()
                sys.exit(1)
            # The probability of crew member not begin present in our current cell
            factor = (1 - prob_mat[initial_node[0]][initial_node[1]])
            # Blocking the current cell probability as it does not contain a crew member
            prob_mat[initial_node[0]][initial_node[1]] = -1
            # Distributing the probability across all open cells if the crew member is not present
